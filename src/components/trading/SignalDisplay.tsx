...
export default function SignalDisplay({ symbol = 'BTCUSDT' }: { symbol?: string }) {
  ...
  useEffect(() => {
    if (connection && isConnected) {
      connection.on('NewSignal', (newSignal: Signal) => {
        setSignal(newSignal);
  ...
  const actionColor = signal.action === 'BUY' ? '#00d4aa' : signal.action === 'SELL' ? '#ff4757' : '#fbbf24';

  return (
    <div style={{ background: '#14141e', border: '1px solid #2a2a3a', borderRadius: '12px', padding: '20px', maxWidth: '350px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
        <span style={{ color: actionColor, fontSize: '28px', fontWeight: '700' }}>
          {signal.action}
        </span>
        <span style={{ color: '#9ca3af', fontSize: '14px' }}>Confidence: {signal.confidence}%</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '14px' }}>
        ...
        {signal.support && (
          <>
            ...
            <span style={{ color: '#ffffff' }}>${signal.support.toLocaleString()}</span>
          </>
        )}
        {signal.resistance && (
          <>
            <span style={{ color: '#9ca3af' }}>Resistance:</span>
            <span style={{ color: '#ffffff' }}>${signal.resistance.toLocaleString()}</span>
          </>
        )}
      </div>

      {signal.reason && (
        <p style={{ color: '#9ca3af', fontSize: '13px', marginTop: '12px', borderTop: '1px solid #2a2a3a', paddingTop: '12px' }}>
          {signal.reason}
        </p>
      )}
    </div>
